#!/usr/bin/env python3
"""
experiment_baseline_and_significance.py
=======================================

Adds the two cheapest, highest-value experiments discussed for the
Cat2Vec-with-positional-encoding classification study:

  1. A RANK-AS-NUMERIC baseline pipeline (Pipeline 0 / "P0").
     This is the simplest order-preserving encoding: each ordinal value is
     mapped to its integer rank and fed in as a single numeric feature
     (then standard-scaled). Comparing P4 against P0 is what isolates how
     much of P4's advantage comes from *sinusoidal positional encoding*
     specifically, versus the mere presence of order information.

  2. A REPEATED-SPLITS + PAIRED-SIGNIFICANCE harness.
     Every (pipeline, model) cell is evaluated over N random train/test
     splits (different seeds). We report mean +/- std for accuracy,
     macro-F1, ROC-AUC and PR-AUC, and run a paired test (paired t-test
     across seeds, plus a McNemar test on the held-out predictions of the
     final seed) comparing the proposed P4 against each other pipeline.

IMPORTANT
---------
This script is written to run on YOUR machine against the real
Cat-in-the-Dat II train.csv. It does not fabricate any results. The
numbers it prints are the ones you should put in the paper.

The Cat2Vec / Cat2Vec_wpos transformers are imported from your existing
repository module (the same code used for Tables IV-VI). If your module
path or class names differ, adjust the import block marked CONFIG below.

SVM WARNING
-----------
A kernel SVM on 225k samples took ~14-19 hours per fit in the original
runs. Running it over multiple seeds is likely infeasible. By default
this script SKIPS the SVM (RUN_SVM = False). Either leave it off, switch
to LinearSVC, or subsample for the SVM only. The other four models are
...
# =====================================================================
# CONFIG -- adjust these to match your repository / data layout
# =====================================================================
DATA_PATH = "../data/train.csv"          # Cat-in-the-Dat II train.csv
TARGET_COL = "target"
ID_COL = "id"                    # dropped if present

# True ordinal variables WITH a meaningful order, plus their rank maps.
# Fill ORDINAL_RANK_MAPS with the same semantic orderings you used for
# Cat2Vec_wpos. For purely numeric ordinals (day, month, ord_0) the
# natural integer order is used automatically if left as None.
...
ORDINAL_RANK_MAPS = {
    ...
    # ord_0, day, month: leave to natural numeric order (handled below)
}

# Nominal variables (incl. binary-as-nominal and the non-ordered ord_3/4/5)
NOMINAL_COLS = (
